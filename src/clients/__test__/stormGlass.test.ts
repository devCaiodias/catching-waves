...
describe('StormGlass client', () => {
  ...
  it('should get a generic error from StormGlass service when the request fail before reaching the service', async () => {
    ...
    await expect(stormGlass.featshPoints(lat, lng)).rejects.toThrow(`Unexpected error when trying to communicate to StormGlass: Network Error`)
  })
  ...
  it('should get an StormGlassResponseError when the stormGlass service response with error', async () => {
    ...
    await expect(stormGlass.featshPoints(lat, lng)).rejects.toThrow('Unexpected error returned by the StormGlass service: Error: {"errors":["Rate Limit reached"]} Code: 429')
  })
})
